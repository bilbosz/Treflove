...
local LoveCursor = {}
---Gets the type of the LoveCursor.
---@return CursorType
function LoveCursor:getType()
end

-- endregion LoveCursor
...
---@return boolean
function m.getRelativeMode()
end

---Gets a LoveCursor object representing a system-native hardware cursor.
---
---Hardware cursors are framerate-independent and work the same way as normal operating system cursors. Unlike drawing an image at the mouse's current coordinates, hardware cursors never have visible lag between when the mouse is moved and when the cursor position updates, even at low framerates.
---@param ctype CursorType @The type of system cursor to get. 
---@return LoveCursor
function m.getSystemCursor(ctype)
end
...
---@return boolean
function m.isDown(button, ...)
end

---Checks if the mouse is grabbed.
---@return boolean
function m.isGrabbed()
end
...
---Creates a new hardware LoveCursor object from an image file or LoveImageData.
---
---Hardware cursors are framerate-independent and work the same way as normal operating system cursors. Unlike drawing an image at the mouse's current coordinates, hardware cursors never have visible lag between when the mouse is moved and when the cursor position updates, even at low framerates.
---
---The hot spot is the point the operating system uses to determine what was clicked and at what position the mouse cursor is. For example, the normal arrow pointer normally has its hot spot at the top left of the image, but a crosshair cursor might have it in the middle.
---@param imageData LoveImageData @The LoveImageData to use for the new LoveCursor.
---@param hotx number @The x-coordinate in the LoveImageData of the cursor's hot spot.
---@param hoty number @The y-coordinate in the LoveImageData of the cursor's hot spot.
---@return LoveCursor
---@overload fun(filename:string, hotx:number, hoty:number):LoveCursor
---@overload fun(fileData:LoveFileData, hotx:number, hoty:number):LoveCursor
function m.newCursor(imageData, hotx, hoty)
end

---Sets the current mouse cursor.
---@param cursor LoveCursor @The LoveCursor object to use as the current mouse cursor.
function m.setCursor(cursor)
end

---Grabs the mouse and confines it to the window.
---@param grab boolean @True to confine the mouse, false to let it leave the window.
function m.setGrabbed(grab)
end
...
function m.setRelativeMode(enable)
end
