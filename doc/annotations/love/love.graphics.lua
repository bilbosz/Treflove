---@class love.graphics
---The primary responsibility for the love.graphics module is the drawing of lines, shapes, text, Images and other LoveDrawable objects onto the screen. Its secondary responsibilities include loading external files (including Images and Fonts) into memory, creating specialized objects (such as ParticleSystems or Canvases) and managing screen geometry.
---
...
-- region LoveCanvas
---@class LoveCanvas
---A LoveCanvas is used for off-screen rendering. Think of it as an invisible screen that you can draw to, but that will not be visible until you draw it to the actual visible screen. It is also known as "render to texture".
---
---By drawing things that do not change position often (such as background items) to the LoveCanvas, and then drawing the entire LoveCanvas instead of each item,  you can reduce the number of draw operations performed each frame.
---
---In versions prior to love.graphics.isSupported("canvas") could be used to check for support at runtime.
local LoveCanvas = {}
---Generates mipmaps for the LoveCanvas, based on the contents of the highest-resolution mipmap level.
---
---The LoveCanvas must be created with mipmaps set to a MipmapMode other than 'none' for this function to work. It should only be called while the LoveCanvas is not the active render target.
---
---If the mipmap mode is set to 'auto', this function is automatically called inside love.graphics.setCanvas when switching from this LoveCanvas to another LoveCanvas or to the main screen.
function LoveCanvas:generateMipmaps()
end

---Gets the number of multisample antialiasing (MSAA) samples used when drawing to the LoveCanvas.
---
---This may be different than the number used as an argument to love.graphics.newCanvas if the system running LÖVE doesn't support that number.
---@return number
function LoveCanvas:getMSAA()
end

---Gets the MipmapMode this LoveCanvas was created with.
---@return MipmapMode
function LoveCanvas:getMipmapMode()
end

---Generates LoveImageData from the contents of the LoveCanvas.
---@return LoveImageData
---@overload fun(slice:number, mipmap:number, x:number, y:number, width:number, height:number):LoveImageData
function LoveCanvas:newImageData()
end

---Render to the LoveCanvas using a function.
---
...
---@return number
function LoveFont:getBaseline()
end

---Gets the DPI scale factor of the LoveFont.
---
...
---@return number
function LoveFont:getDPIScale()
end

---Gets the descent of the LoveFont.
---
---The descent spans the distance between the baseline and the lowest descending glyph in a typeface.
---@return number
function LoveFont:getDescent()
end

---Gets the filter mode for a font.
---@return FilterMode, FilterMode, number
function LoveFont:getFilter()
end

---Gets the height of the LoveFont.
---
...
---
---This will be the value previously set by LoveFont:setLineHeight, or 1.0 by default.
---@return number
function LoveFont:getLineHeight()
end

---Determines the maximum width (accounting for newlines) taken by the given string.
---@param text string @A string.
---@return number
function LoveFont:getWidth(text)
end
...
---@return number, table
function LoveFont:getWrap(text, wraplimit)
end

---Gets whether the LoveFont can render a character or string.
...
function LoveFont:hasGlyphs(text)
end

---Sets the fallback fonts. When the LoveFont doesn't contain a glyph, it will substitute the glyph from the next subsequent fallback Fonts. This is akin to setting a 'font stack' in Cascading Style Sheets (CSS).
---@param fallbackfont1 LoveFont @The first fallback LoveFont to use.
---@param ... LoveFont @Additional fallback Fonts.
function LoveFont:setFallbacks(fallbackfont1, ...)
end
...
function LoveFont:setFilter(min, mag, anisotropy)
end
...
function LoveFont:setLineHeight(height)
end

-- endregion LoveFont
-- region LoveImage
---@class LoveImage
---LoveDrawable image type.
local LoveImage = {}
---Gets the flags used when the image was created.
---@return table
function LoveImage:getFlags()
end

---Gets whether the LoveImage was created from CompressedData.
---
...
---@return boolean
function LoveImage:isCompressed()
end

---Replace the contents of an LoveImage.
---@param data LoveImageData @The new LoveImageData to replace the contents with.
---@param slice number @Which cubemap face, array index, or volume layer to replace, if applicable.
---@param mipmap number @The mimap level to replace, if the LoveImage has mipmaps.
---@param x number @The x-offset in pixels from the top-left of the image to replace. The given LoveImageData's width plus this value must not be greater than the pixel width of the LoveImage's specified mipmap level.
---@param y number @The y-offset in pixels from the top-left of the image to replace. The given LoveImageData's height plus this value must not be greater than the pixel height of the LoveImage's specified mipmap level.
---@param reloadmipmaps boolean @Whether to generate new mipmaps after replacing the LoveImage's pixels. True by default if the LoveImage was created with automatically generated mipmaps, false by default otherwise.
function LoveImage:replacePixels(data, slice, mipmap, x, y, reloadmipmaps)
end
...
local LoveMesh = {}
---Attaches a vertex attribute from a different LoveMesh onto this LoveMesh, for use when drawing. This can be used to share vertex attribute data between several different Meshes.
---@param name string @The name of the vertex attribute to attach.
---@param mesh LoveMesh @The LoveMesh to get the vertex attribute from.
---@overload fun(name:string, mesh:LoveMesh, step:VertexAttributeStep, attachname:string):void
function LoveMesh:attachAttribute(name, mesh)
end
...
---Removes a previously attached vertex attribute from this LoveMesh.
---@param name string @The name of the attached vertex attribute to detach.
---@return boolean
function LoveMesh:detachAttribute(name)
end

---Gets the mode used when drawing the LoveMesh.
---@return MeshDrawMode
function LoveMesh:getDrawMode()
end

---Gets the range of vertices used when drawing the LoveMesh.
---@return number, number
function LoveMesh:getDrawRange()
end

---Gets the texture (LoveImage or LoveCanvas) used when drawing the LoveMesh.
---@return LoveTexture
function LoveMesh:getTexture()
end

---Gets the properties of a vertex in the LoveMesh.
---
...
function LoveMesh:getVertex(index)
end

---Gets the properties of a specific attribute within a vertex in the LoveMesh.
---
...
function LoveMesh:getVertexAttribute(vertexindex, attributeindex)
end

---Gets the total number of vertices in the LoveMesh.
---@return number
function LoveMesh:getVertexCount()
end

---Gets the vertex format that the LoveMesh was created with.
---@return table
function LoveMesh:getVertexFormat()
end

---Gets the vertex map for the LoveMesh. The vertex map describes the order in which the vertices are used when the LoveMesh is drawn. The vertices, vertex map, and mesh draw mode work together to determine what exactly is displayed on the screen.
---
---If no vertex map has been set previously via LoveMesh:setVertexMap, then this function will return nil in LÖVE 0.10.0+, or an empty table in 0.9.2 and older.
---@return table
function LoveMesh:getVertexMap()
end

---Gets whether a specific vertex attribute in the LoveMesh is enabled. Vertex data from disabled attributes is not used when drawing the LoveMesh.
---@param name string @The name of the vertex attribute to be checked.
---@return boolean
function LoveMesh:isAttributeEnabled(name)
end

---Enables or disables a specific vertex attribute in the LoveMesh. Vertex data from disabled attributes is not used when drawing the LoveMesh.
---@param name string @The name of the vertex attribute to enable or disable.
---@param enable boolean @Whether the vertex attribute is used when drawing this LoveMesh.
function LoveMesh:setAttributeEnabled(name, enable)
end

---Sets the mode used when drawing the LoveMesh.
---@param mode MeshDrawMode @The mode to use when drawing the LoveMesh.
function LoveMesh:setDrawMode(mode)
end

---Restricts the drawn vertices of the LoveMesh to a subset of the total.
---@param start number @The index of the first vertex to use when drawing, or the index of the first value in the vertex map to use if one is set for this LoveMesh.
---@param count number @The number of vertices to use when drawing, or number of values in the vertex map to use if one is set for this LoveMesh.
function LoveMesh:setDrawRange(start, count)
end

---Sets the texture (LoveImage or LoveCanvas) used when drawing the LoveMesh.
---@param texture LoveTexture @The LoveImage or LoveCanvas to texture the LoveMesh with when drawing.
function LoveMesh:setTexture(texture)
end

---Sets the properties of a vertex in the LoveMesh.
---
---In versions prior to 11.0, color and byte component values were within the range of 0 to 255 instead of 0 to 1.
...
---@overload fun(index:number, vertex:table):void
function LoveMesh:setVertex(index, attributecomponent, ...)
end

---Sets the properties of a specific attribute within a vertex in the LoveMesh.
---
---Meshes without a custom vertex format specified in love.graphics.newMesh have position as their first attribute, texture coordinates as their second attribute, and color as their third attribute.
...
function LoveMesh:setVertexAttribute(vertexindex, attributeindex, value1, value2, ...)
end

---Sets the vertex map for the LoveMesh. The vertex map describes the order in which the vertices are used when the LoveMesh is drawn. The vertices, vertex map, and mesh draw mode work together to determine what exactly is displayed on the screen.
---
---The vertex map allows you to re-order or reuse vertices when drawing without changing the actual vertex parameters or duplicating vertices. It is especially useful when combined with different LoveMesh draw Modes.
---@param map table @A table containing a list of vertex indices to use when drawing. Values must be in the range of LoveMesh:getVertexCount().
---@overload fun(vi1:number, vi2:number, vi3:number):void
---@overload fun(data:LoveData, datatype:IndexDataType):void
function LoveMesh:setVertexMap(map)
end

---Replaces a range of vertices in the LoveMesh with new ones. The total number of vertices in a LoveMesh cannot be changed after it has been created. This is often more efficient than calling LoveMesh:setVertex in a loop.
...
---@overload fun(data:LoveData, startvertex:number):void
---@overload fun(vertices:table):void
function LoveMesh:setVertices(vertices, startvertex)
end

-- endregion LoveMesh
-- region LoveParticleSystem
---@class LoveParticleSystem
---A LoveParticleSystem can be used to create particle effects like fire or smoke.
---
...
local LoveParticleSystem = {}
---Creates an identical copy of the LoveParticleSystem in the stopped state.
---@return LoveParticleSystem
function LoveParticleSystem:clone()
end
...
function LoveParticleSystem:emit(numparticles)
end

---Gets the maximum number of particles the LoveParticleSystem can have at once.
---@return number
function LoveParticleSystem:getBufferSize()
end
...
function LoveParticleSystem:getColors()
end

---Gets the number of particles that are currently in the system.
---@return number
function LoveParticleSystem:getCount()
end

---Gets the direction of the particle emitter (in radians).
---@return number
function LoveParticleSystem:getDirection()
end

---Gets the area-based spawn parameters for the particles.
---@return AreaSpreadDistribution, number, number, number, boolean
function LoveParticleSystem:getEmissionArea()
end

---Gets the amount of particles emitted per second.
---@return number
function LoveParticleSystem:getEmissionRate()
end

---Gets how long the particle system will emit particles (if -1 then it emits particles forever).
---@return number
function LoveParticleSystem:getEmitterLifetime()
end

---Gets the mode used when the LoveParticleSystem adds new particles.
---@return ParticleInsertMode
function LoveParticleSystem:getInsertMode()
end
...
function LoveParticleSystem:getLinearAcceleration()
end

---Gets the amount of linear damping (constant deceleration) for particles.
---@return number, number
function LoveParticleSystem:getLinearDamping()
end

---Gets the particle image's draw offset.
---@return number, number
function LoveParticleSystem:getOffset()
end

---Gets the lifetime of the particles.
---@return number, number
function LoveParticleSystem:getParticleLifetime()
end

---Gets the position of the emitter.
---@return number, number
function LoveParticleSystem:getPosition()
end

---Gets the series of Quads used for the particle sprites.
---@return table
function LoveParticleSystem:getQuads()
end

---Gets the radial acceleration (away from the emitter).
---@return number, number
function LoveParticleSystem:getRadialAcceleration()
end

---Gets the rotation of the image upon particle creation (in radians).
---@return number, number
function LoveParticleSystem:getRotation()
end

---Gets the amount of size variation (0 meaning no variation and 1 meaning full variation between start and end).
---@return number
function LoveParticleSystem:getSizeVariation()
end
...
---@return number, number, number
function LoveParticleSystem:getSizes()
end

---Gets the speed of the particles.
---@return number, number
function LoveParticleSystem:getSpeed()
end

---Gets the spin of the sprite.
---@return number, number, number
function LoveParticleSystem:getSpin()
end

---Gets the amount of spin variation (0 meaning no variation and 1 meaning full variation between start and end).
---@return number
function LoveParticleSystem:getSpinVariation()
end

---Gets the amount of directional spread of the particle emitter (in radians).
---@return number
function LoveParticleSystem:getSpread()
end

---Gets the tangential acceleration (acceleration perpendicular to the particle's direction).
---@return number, number
function LoveParticleSystem:getTangentialAcceleration()
end

---Gets the texture (LoveImage or LoveCanvas) used for the particles.
---@return LoveTexture
function LoveParticleSystem:getTexture()
end
...
---@return boolean
function LoveParticleSystem:hasRelativeRotation()
end

---Checks whether the particle system is actively emitting particles.
---@return boolean
function LoveParticleSystem:isActive()
end

---Checks whether the particle system is paused.
---@return boolean
function LoveParticleSystem:isPaused()
end

---Checks whether the particle system is stopped.
---@return boolean
function LoveParticleSystem:isStopped()
end

---Moves the position of the emitter. This results in smoother particle spawning behaviour than if LoveParticleSystem:setPosition is used every frame.
---@param x number @Position along x-axis.
---@param y number @Position along y-axis.
function LoveParticleSystem:moveTo(x, y)
end

---Pauses the particle emitter.
function LoveParticleSystem:pause()
end

---Resets the particle emitter, removing any existing particles and resetting the lifetime counter.
function LoveParticleSystem:reset()
end

---Sets the size of the buffer (the max allowed amount of particles in the system).
---@param size number @The buffer size.
function LoveParticleSystem:setBufferSize(size)
end
...
function LoveParticleSystem:setColors(r1, g1, b1, a1, r2, g2, b2, a2, r8, g8, b8, a8)
end

---Sets the direction the particles will be emitted in.
---@param direction number @The direction of the particles (in radians).
function LoveParticleSystem:setDirection(direction)
end
...
function LoveParticleSystem:setEmissionArea(distribution, dx, dy, angle, directionRelativeToCenter)
end

---Sets the amount of particles emitted per second.
---@param rate number @The amount of particles per second.
function LoveParticleSystem:setEmissionRate(rate)
end

---Sets how long the particle system should emit particles (if -1 then it emits particles forever).
---@param life number @The lifetime of the emitter (in seconds).
function LoveParticleSystem:setEmitterLifetime(life)
end

---Sets the mode to use when the LoveParticleSystem adds new particles.
---@param mode ParticleInsertMode @The mode to use when the LoveParticleSystem adds new particles.
function LoveParticleSystem:setInsertMode(mode)
end
...
function LoveParticleSystem:setLinearAcceleration(xmin, ymin, xmax, ymax)
end
...
function LoveParticleSystem:setLinearDamping(min, max)
end
...
function LoveParticleSystem:setOffset(x, y)
end
...
function LoveParticleSystem:setParticleLifetime(min, max)
end

---Sets the position of the emitter.
---@param x number @Position along x-axis.
---@param y number @Position along y-axis.
function LoveParticleSystem:setPosition(x, y)
end

---Sets a series of Quads to use for the particle sprites. Particles will choose a LoveQuad from the list based on the particle's current lifetime, allowing for the use of animated sprite sheets with ParticleSystems.
---@param quad1 LoveQuad @The first LoveQuad to use.
---@param quad2 LoveQuad @The second LoveQuad to use.
---@overload fun(quads:table):void
function LoveParticleSystem:setQuads(quad1, quad2)
end

---Set the radial acceleration (away from the emitter).
---@param min number @The minimum acceleration.
---@param max number @The maximum acceleration.
function LoveParticleSystem:setRadialAcceleration(min, max)
end
...
function LoveParticleSystem:setSizeVariation(variation)
end
...
function LoveParticleSystem:setSizes(size1, size2, size8)
end
...
function LoveParticleSystem:setSpeed(min, max)
end

---Sets the spin of the sprite.
---@param min number @The minimum spin (radians per second).
---@param max number @The maximum spin (radians per second).
function LoveParticleSystem:setSpin(min, max)
end

---Sets the amount of spin variation (0 meaning no variation and 1 meaning full variation between start and end).
---@param variation number @The amount of variation (0 meaning no variation and 1 meaning full variation between start and end).
function LoveParticleSystem:setSpinVariation(variation)
end

---Sets the amount of spread for the system.
---@param spread number @The amount of spread (radians).
function LoveParticleSystem:setSpread(spread)
end

---Sets the tangential acceleration (acceleration perpendicular to the particle's direction).
---@param min number @The minimum acceleration.
---@param max number @The maximum acceleration.
function LoveParticleSystem:setTangentialAcceleration(min, max)
end

---Sets the texture (LoveImage or LoveCanvas) to be used for the particles.
---@param texture LoveTexture @An LoveImage or LoveCanvas to use for the particles.
function LoveParticleSystem:setTexture(texture)
end

---Starts the particle emitter.
function LoveParticleSystem:start()
end

---Stops the particle emitter, resetting the lifetime counter.
function LoveParticleSystem:stop()
end

---Updates the particle system; moving, creating and killing particles.
---@param dt number @The time (seconds) since last frame.
function LoveParticleSystem:update(dt)
end

-- endregion LoveParticleSystem
-- region LoveQuad
---@class LoveQuad
...
---@return number, number
function LoveQuad:getTextureDimensions()
end

---Gets the current viewport of this LoveQuad.
---@return number, number, number, number
function LoveQuad:getViewport()
end
...
---@param sw number @The reference width, the width of the LoveImage. (Must be greater than 0.)
---@param sh number @The reference height, the height of the LoveImage. (Must be greater than 0.)
function LoveQuad:setViewport(x, y, w, h, sw, sh)
end

-- endregion LoveQuad
-- region LoveShader
---@class LoveShader
---A LoveShader is used for advanced hardware-accelerated pixel or vertex manipulation. These effects are written in a language based on GLSL (OpenGL Shading Language) with a few things simplified for easier coding.
---
...
---@return string
function LoveShader:getWarnings()
end

---Gets whether a uniform / extern variable exists in the LoveShader.
---
...
---@return boolean
function LoveShader:hasUniform(name)
end
...
---
---Uniform / extern variables are read-only in the shader code and remain constant until modified by a LoveShader:send call. Uniform variables can be accessed in both the Vertex and Pixel components of a shader, as long as the variable is declared in each.
...
---@overload fun(name:string, texture:LoveTexture):void
...
---@overload fun(name:string, data:LoveData, offset:number, size:number):void
---@overload fun(name:string, data:LoveData, matrixlayout:MatrixLayout, offset:number, size:number):void
function LoveShader:send(name, number, ...)
end
...
function LoveShader:sendColor(name, color, ...)
end

-- endregion LoveShader
-- region LoveSpriteBatch
...
---
---A LoveSpriteBatch can be even more useful when the underlying image is a texture atlas (a single image file containing many independent images); by adding Quads to the batch, different sub-images from within the atlas can be drawn.
local LoveSpriteBatch = {}
...
---@return number
---@overload fun(quad:LoveQuad, x:number, y:number, r:number, sx:number, sy:number, ox:number, oy:number, kx:number, ky:number):number
function LoveSpriteBatch:add(x, y, r, sx, sy, ox, oy, kx, ky)
end

---Adds a sprite to a batch created with an Array LoveTexture.
...
---@return number
---@overload fun(layerindex:number, quad:LoveQuad, x:number, y:number, r:number, sx:number, sy:number, ox:number, oy:number, kx:number, ky:number):number
---@overload fun(layerindex:number, transform:LoveTransform):number
---@overload fun(layerindex:number, quad:LoveQuad, transform:LoveTransform):number
function LoveSpriteBatch:addLayer(layerindex, x, y, r, sx, sy, ox, oy, kx, ky)
end

---Attaches a per-vertex attribute from a LoveMesh onto this LoveSpriteBatch, for use when drawing. This can be combined with a LoveShader to augment a LoveSpriteBatch with per-vertex or additional per-sprite information instead of just having per-sprite colors.
---
---Each sprite in a LoveSpriteBatch has 4 vertices in the following order: top-left, bottom-left, top-right, bottom-right. The index returned by LoveSpriteBatch:add (and used by LoveSpriteBatch:set) can used to determine the first vertex of a specific sprite with the formula 1 + 4 * ( id - 1 ).
---@param name string @The name of the vertex attribute to attach.
---@param mesh LoveMesh @The LoveMesh to get the vertex attribute from.
function LoveSpriteBatch:attachAttribute(name, mesh)
end

---Removes all sprites from the buffer.
function LoveSpriteBatch:clear()
end

---Immediately sends all new and modified sprite data in the batch to the graphics card.
---
---Normally it isn't necessary to call this method as love.graphics.draw(spritebatch, ...) will do it automatically if needed, but explicitly using LoveSpriteBatch:flush gives more control over when the work happens.
---
...
function LoveSpriteBatch:flush()
end

---Gets the maximum number of sprites the LoveSpriteBatch can hold.
---@return number
function LoveSpriteBatch:getBufferSize()
end

---Gets the color that will be used for the next add and set operations.
---
---If no color has been set with LoveSpriteBatch:setColor or the current LoveSpriteBatch color has been cleared, this method will return nil.
---
---In versions prior to 11.0, color component values were within the range of 0 to 255 instead of 0 to 1.
---@return number, number, number, number
function LoveSpriteBatch:getColor()
end

---Gets the number of sprites currently in the LoveSpriteBatch.
---@return number
function LoveSpriteBatch:getCount()
end

---Gets the texture (LoveImage or LoveCanvas) used by the LoveSpriteBatch.
---@return LoveTexture
function LoveSpriteBatch:getTexture()
end

---Changes a sprite in the batch. This requires the sprite index returned by LoveSpriteBatch:add or LoveSpriteBatch:addLayer.
...
---@param kx number @Shear factor (x-axis).
---@param ky number @Shear factor (y-axis).
---@overload fun(spriteindex:number, quad:LoveQuad, x:number, y:number, r:number, sx:number, sy:number, ox:number, oy:number, kx:number, ky:number):void
function LoveSpriteBatch:set(spriteindex, x, y, r, sx, sy, ox, oy, kx, ky)
end

---Sets the color that will be used for the next add and set operations. Calling the function without arguments will disable all per-sprite colors for the LoveSpriteBatch.
---
---In versions prior to 11.0, color component values were within the range of 0 to 255 instead of 0 to 1.
---
---In version 0.9.2 and older, the global color set with love.graphics.setColor will not work on the LoveSpriteBatch if any of the sprites has its own color.
...
function LoveSpriteBatch:setColor(r, g, b, a)
end

---Restricts the drawn sprites in the LoveSpriteBatch to a subset of the total.
---@param start number @The index of the first sprite to draw. Index 1 corresponds to the first sprite added with LoveSpriteBatch:add.
---@param count number @The number of sprites to draw.
function LoveSpriteBatch:setDrawRange(start, count)
end

---Changes a sprite previously added with add or addLayer, in a batch created with an Array LoveTexture.
---@param spriteindex number @The index of the existing sprite to replace.
---@param layerindex number @The index of the layer in the Array LoveTexture to use for this sprite.
---@param x number @The position to draw the sprite (x-axis).
---@param y number @The position to draw the sprite (y-axis).
---@param r number @Orientation (radians).
---@param sx number @Scale factor (x-axis).
...
---@param kx number @Shearing factor (x-axis).
---@param ky number @Shearing factor (y-axis).
---@overload fun(spriteindex:number, layerindex:number, quad:LoveQuad, x:number, y:number, r:number, sx:number, sy:number, ox:number, oy:number, kx:number, ky:number):void
---@overload fun(spriteindex:number, layerindex:number, transform:LoveTransform):void
---@overload fun(spriteindex:number, layerindex:number, quad:LoveQuad, transform:LoveTransform):void
function LoveSpriteBatch:setLayer(spriteindex, layerindex, x, y, r, sx, sy, ox, oy, kx, ky)
end

---Sets the texture (LoveImage or LoveCanvas) used for the sprites in the batch, when drawing.
---@param texture LoveTexture @The new LoveImage or LoveCanvas to use for the sprites in the batch.
function LoveSpriteBatch:setTexture(texture)
end

-- endregion LoveSpriteBatch
-- region LoveText
---@class LoveText
---LoveDrawable text.
local LoveText = {}
---Adds additional colored text to the LoveText object at the specified position.
...
function LoveText:add(textstring, x, y, angle, sx, sy, ox, oy, kx, ky)
end

---Adds additional formatted / colored text to the LoveText object at the specified position.
---
...
function LoveText:addf(textstring, wraplimit, align, x, y, angle, sx, sy, ox, oy, kx, ky)
end

---Clears the contents of the LoveText object.
...
function LoveText:getDimensions()
end

---Gets the LoveFont used with the LoveText object.
---@return LoveFont
function LoveText:getFont()
end

---Gets the height of the text in pixels.
---@return number
---@overload fun(index:number):number
function LoveText:getHeight()
end

---Gets the width of the text in pixels.
---@return number
---@overload fun(index:number):number
function LoveText:getWidth()
end

---Replaces the contents of the LoveText object with a new unformatted string.
---@param textstring string @The new string of text to use.
---@overload fun(coloredtext:table):void
function LoveText:set(textstring)
end

---Replaces the LoveFont used with the text.
---@param font LoveFont @The new font to use with this LoveText object.
function LoveText:setFont(font)
end

---Replaces the contents of the LoveText object with a new formatted string.
---@param textstring string @The new string of text to use.
---@param wraplimit number @The maximum width in pixels of the text before it gets automatically wrapped to a new line.
---@param align AlignMode @The alignment of the text.
---@overload fun(coloredtext:table, wraplimit:number, align:AlignMode):void
function LoveText:setf(textstring, wraplimit, align)
end
...
local LoveTexture = {}
---Gets the DPI scale factor of the LoveTexture.
---
...
---@return number
function LoveTexture:getDPIScale()
end

---Gets the depth of a Volume LoveTexture. Returns 1 for 2D, Cubemap, and Array textures.
---@return number
function LoveTexture:getDepth()
end
...
---@return CompareMode
function LoveTexture:getDepthSampleMode()
end

---Gets the width and height of the LoveTexture.
---@return number, number
function LoveTexture:getDimensions()
end

---Gets the filter mode of the LoveTexture.
---@return FilterMode, FilterMode, number
function LoveTexture:getFilter()
end

---Gets the pixel format of the LoveTexture.
---@return PixelFormat
function LoveTexture:getFormat()
end

---Gets the height of the LoveTexture.
---@return number
function LoveTexture:getHeight()
end

---Gets the number of layers / slices in an Array LoveTexture. Returns 1 for 2D, Cubemap, and Volume textures.
---@return number
function LoveTexture:getLayerCount()
end

---Gets the number of mipmaps contained in the LoveTexture. If the texture was not created with mipmaps, it will return 1.
---@return number
function LoveTexture:getMipmapCount()
end

---Gets the mipmap filter mode for a LoveTexture. Prior to 11.0 this method only worked on Images.
---@return FilterMode, number
function LoveTexture:getMipmapFilter()
end

---Gets the width and height in pixels of the LoveTexture.
---
---LoveTexture:getDimensions gets the dimensions of the texture in units scaled by the texture's DPI scale factor, rather than pixels. Use getDimensions for calculations related to drawing the texture (calculating an origin offset, for example), and getPixelDimensions only when dealing specifically with pixels, for example when using LoveCanvas:newImageData.
---@return number, number
function LoveTexture:getPixelDimensions()
end

---Gets the height in pixels of the LoveTexture.
---
---DPI scale factor, rather than pixels. Use getHeight for calculations related to drawing the texture (calculating an origin offset, for example), and getPixelHeight only when dealing specifically with pixels, for example when using LoveCanvas:newImageData.
---@return number
function LoveTexture:getPixelHeight()
end

---Gets the width in pixels of the LoveTexture.
---
---DPI scale factor, rather than pixels. Use getWidth for calculations related to drawing the texture (calculating an origin offset, for example), and getPixelWidth only when dealing specifically with pixels, for example when using LoveCanvas:newImageData.
---@return number
function LoveTexture:getPixelWidth()
end

---Gets the type of the LoveTexture.
---@return TextureType
function LoveTexture:getTextureType()
end

---Gets the width of the LoveTexture.
---@return number
function LoveTexture:getWidth()
end

---Gets the wrapping properties of a LoveTexture.
---
...
function LoveTexture:getWrap()
end

---Gets whether the LoveTexture can be drawn and sent to a LoveShader.
---
...
---@return boolean
function LoveTexture:isReadable()
end
...
---
---When using a depth texture with a comparison mode set in a shader, it must be declared as a sampler2DShadow and used in a GLSL 3 LoveShader. The result of accessing the texture in the shader will return a float between 0 and 1, proportional to the number of samples (up to 4 samples will be used if bilinear filtering is enabled) that passed the test set by the comparison operation.
---
...
function LoveTexture:setDepthSampleMode(compare)
end

---Sets the filter mode of the LoveTexture.
...
function LoveTexture:setFilter(min, mag, anisotropy)
end

---Sets the mipmap filter mode for a LoveTexture. Prior to 11.0 this method only worked on Images.
---
...
function LoveTexture:setMipmapFilter(filtermode, sharpness)
end

---Sets the wrapping properties of a LoveTexture.
---
---This function sets the way a LoveTexture is repeated when it is drawn with a LoveQuad that is larger than the texture's extent, or when a custom LoveShader is used which uses texture coordinates outside of [0, 1]. A texture may be clamped or set to repeat in both horizontal and vertical directions.
---
---Clamped textures appear only once (with the edges of the texture stretching to fill the extent of the LoveQuad), whereas repeated ones repeat as many times as there is room in the LoveQuad.
...
function LoveTexture:setWrap(horiz, vert, depth)
end

-- endregion LoveTexture
-- region LoveVideo
---@class LoveVideo
---A drawable video.
local LoveVideo = {}
---Gets the width and height of the LoveVideo in pixels.
---@return number, number
function LoveVideo:getDimensions()
end

---Gets the scaling filters used when drawing the LoveVideo.
---@return FilterMode, FilterMode, number
function LoveVideo:getFilter()
end

---Gets the height of the LoveVideo in pixels.
---@return number
function LoveVideo:getHeight()
end

---Gets the audio LoveSource used for playing back the video's audio. May return nil if the video has no audio, or if LoveVideo:setSource is called with a nil argument.
---@return LoveSource
function LoveVideo:getSource()
end

---Gets the LoveVideoStream object used for decoding and controlling the video.
---@return LoveVideoStream
function LoveVideo:getStream()
end

---Gets the width of the LoveVideo in pixels.
---@return number
function LoveVideo:getWidth()
end

---Gets whether the LoveVideo is currently playing.
---@return boolean
function LoveVideo:isPlaying()
end

---Pauses the LoveVideo.
function LoveVideo:pause()
end

---Starts playing the LoveVideo. In order for the video to appear onscreen it must be drawn with love.graphics.draw.
function LoveVideo:play()
end

---Rewinds the LoveVideo to the beginning.
function LoveVideo:rewind()
end

---Sets the current playback position of the LoveVideo.
---@param offset number @The time in seconds since the beginning of the LoveVideo.
function LoveVideo:seek(offset)
end

---Sets the scaling filters used when drawing the LoveVideo.
---@param min FilterMode @The filter mode used when scaling the LoveVideo down.
---@param mag FilterMode @The filter mode used when scaling the LoveVideo up.
---@param anisotropy number @Maximum amount of anisotropic filtering used.
function LoveVideo:setFilter(min, mag, anisotropy)
end

---Sets the audio LoveSource used for playing back the video's audio. The audio LoveSource also controls playback speed and synchronization.
---@param source LoveSource @The audio LoveSource used for audio playback, or nil to disable audio synchronization.
function LoveVideo:setSource(source)
end

---Gets the current playback position of the LoveVideo.
---@return number
function LoveVideo:tell()
end
...
---How LoveMesh geometry is culled when rendering.
CullMode = {
...
GraphicsLimit = {
    ...
    ["multicanvas"] = 3,
    ---The maximum number of antialiasing samples for a LoveCanvas.
    ["canvasmsaa"] = 4,
    ...
    ["cubetexturesize"] = 7,
    ---The maximum amount of anisotropic filtering. LoveTexture:setMipmapFilter internally clamps the given anisotropy value to the system's limit.
    ["anisotropy"] = 8
}
---Vertex map datatype for LoveData variant of LoveMesh:setVertexMap.
IndexDataType = {
...
---How a LoveMesh's vertices are used when drawing.
MeshDrawMode = {
...
---Controls whether a LoveCanvas has mipmaps, and its behaviour when it does.
MipmapMode = {
    ---The LoveCanvas has no mipmaps.
    ["none"] = 1,
    ---The LoveCanvas has mipmaps. love.graphics.setCanvas can be used to render to a specific mipmap level, or LoveCanvas:generateMipmaps can (re-)compute all mipmap levels based on the base level.
    ["auto"] = 2,
    ---The LoveCanvas has mipmaps, and all mipmap levels will automatically be recomputed when switching away from the LoveCanvas with love.graphics.setCanvas.
    ["manual"] = 3
}
---How newly created particles are added to the LoveParticleSystem.
ParticleInsertMode = {
    ---Particles are inserted at the top of the LoveParticleSystem's list of particles.
    ["top"] = 1,
    ---Particles are inserted at the bottom of the LoveParticleSystem's list of particles.
    ["bottom"] = 2,
    ---Particles are inserted at random positions in the LoveParticleSystem's list of particles.
    ["random"] = 3
}
...
TextureType = {
    ...
    ["array"] = 2,
    ---Cubemap texture with 6 faces. Requires a custom shader (and LoveShader:send) to use. Sampling from a cube texture in a shader takes a 3D direction vector instead of a texture coordinate.
    ["cube"] = 3,
...
---The frequency at which a vertex shader fetches the vertex attribute's data from the LoveMesh when it's drawn.
---
---Per-instance attributes can be used to render a LoveMesh many times with different positions, colors, or other attributes via a single love.graphics.drawInstanced call, without using the love_InstanceID vertex shader variable.
VertexAttributeStep = {
    ---The vertex attribute will have a unique value for each vertex in the LoveMesh.
    ["pervertex"] = 1,
    ---The vertex attribute will have a unique value for each instance of the LoveMesh.
    ["perinstance"] = 2
}
---How LoveMesh geometry vertices are ordered.
VertexWinding = {
    ---Clockwise.
    ["cw"] = 1,
    ---Counter-clockwise.
...
---How the image wraps inside a LoveQuad with a larger quad size than image size. This also affects how Meshes with texture coordinates which are outside the range of 1 are drawn, and the color returned by the Texel LoveShader function when using it to sample from texture coordinates outside of the range of 1.
WrapMode = {
...
---Applies the given LoveTransform object to the current coordinate transformation.
---
---This effectively multiplies the existing coordinate transformation's matrix with the LoveTransform object's internal matrix to produce the new coordinate transformation.
---@param transform LoveTransform @The LoveTransform object to apply to the current graphics coordinate transform.
function m.applyTransform(transform)
end
...
function m.arc(drawmode, x, y, radius, angle1, angle2, segments)
end
...
---@overload fun(channel:LoveChannel):void
function m.captureScreenshot(filename)
end
...
---Clears the screen or active LoveCanvas to the specified color.
---
...
---Discards (trashes) the contents of the screen or active LoveCanvas. This is a performance optimization function with niche use cases.
---
---If the active LoveCanvas has just been changed and the 'replace' BlendMode is about to be used to draw something which covers the entire screen, calling love.graphics.discard rather than calling love.graphics.clear or doing nothing may improve performance on mobile devices.
---
---On some desktop systems this function may do nothing.
---@param discardcolor boolean @Whether to discard the texture(s) of the active LoveCanvas(es) (the contents of the screen if no LoveCanvas is active.)
---@param discardstencil boolean @Whether to discard the contents of the stencil buffer of the screen / active LoveCanvas.
---@overload fun(discardcolors:table, discardstencil:boolean):void
function m.discard(discardcolor, discardstencil)
end

---Draws a LoveDrawable object (an LoveImage, LoveCanvas, LoveSpriteBatch, LoveParticleSystem, LoveMesh, LoveText object, or LoveVideo) on the screen with optional rotation, scaling and shearing.
---
---Objects are drawn relative to their local coordinate system. The origin is by default located at the top left corner of LoveImage and LoveCanvas. All scaling, shearing, and rotation arguments transform the object relative to that point. Also, the position of the origin can be specified on the screen coordinate system.
---
...
---@param drawable LoveDrawable @A drawable object.
---@param x number @The position to draw the object (x-axis).
---@param y number @The position to draw the object (y-axis).
---@param r number @Orientation (radians).
---@param sx number @Scale factor (x-axis).
...
---@param kx number @Shearing factor (x-axis).
---@param ky number @Shearing factor (y-axis).
---@overload fun(texture:LoveTexture, quad:LoveQuad, x:number, y:number, r:number, sx:number, sy:number, ox:number, oy:number, kx:number, ky:number):void
---@overload fun(drawable:LoveDrawable, transform:LoveTransform):void
---@overload fun(texture:LoveTexture, quad:LoveQuad, transform:LoveTransform):void
function m.draw(drawable, x, y, r, sx, sy, ox, oy, kx, ky)
end

---Draws many instances of a LoveMesh with a single draw call, using hardware geometry instancing.
---
...
---@param mesh LoveMesh @The mesh to render.
...
---@param kx number @Shearing factor (x-axis).
---@param ky number @Shearing factor (y-axis).
---@overload fun(mesh:LoveMesh, instancecount:number, transform:LoveTransform):void
function m.drawInstanced(mesh, instancecount, x, y, r, sx, sy, ox, oy, kx, ky)
end

---Draws a layer of an Array LoveTexture.
---@param texture LoveTexture @The Array LoveTexture to draw.
...
---@param kx number @Shearing factor (x-axis).
---@param ky number @Shearing factor (y-axis).
---@overload fun(texture:LoveTexture, layerindex:number, quad:LoveQuad, x:number, y:number, r:number, sx:number, sy:number, ox:number, oy:number, kx:number, ky:number):void
---@overload fun(texture:LoveTexture, layerindex:number, transform:LoveTransform):void
---@overload fun(texture:LoveTexture, layerindex:number, quad:LoveQuad, transform:LoveTransform):void
function m.drawLayer(texture, layerindex, x, y, r, sx, sy, ox, oy, kx, ky)
end
...
function m.ellipse(mode, x, y, radiusx, radiusy)
end
...
---
---The current batch will be automatically flushed by color), as well as LoveShader:send and methods on Textures which change their state. Using a different LoveImage in consecutive love.graphics.draw calls will also flush the current batch.
---
---SpriteBatches, ParticleSystems, Meshes, and LoveText objects do their own batching and do not affect automatic batching of other draws, aside from flushing the current batch when they're drawn.
function m.flushBatch()
end
...
function m.getBlendMode()
end

---Gets the current target LoveCanvas.
---@return LoveCanvas
function m.getCanvas()
end

---Gets the available LoveCanvas formats, and whether each is supported.
---@return table
---@overload fun(readable:boolean):table
function m.getCanvasFormats()
end
...
---
---The color mask determines whether individual components of the colors of drawn objects will affect the color of the screen. They affect love.graphics.clear and LoveCanvas:clear as well.
---@return boolean, boolean, boolean, boolean
function m.getColorMask()
end
...
---Gets the current LoveFont object.
---@return LoveFont
function m.getFont()
end
...
---
---This is designed for use in combination with LoveMesh face culling. Other love.graphics shapes, lines, and sprites are not guaranteed to have a specific winding order to their internal vertices.
---@return VertexWinding
function m.getFrontFaceWinding()
end
...
---@return table
function m.getImageFormats()
end

---Gets the line join style.
---@return LineJoin
function m.getLineJoin()
end

---Gets the line style.
---@return LineStyle
function m.getLineStyle()
end

---Gets the current line width.
---@return number
function m.getLineWidth()
end

---Gets whether back-facing triangles in a LoveMesh are culled.
---
---LoveMesh face culling is designed for use with low level custom hardware-accelerated 3D rendering via custom vertex attributes on Meshes, custom vertex shaders, and depth testing with a depth buffer.
---@return CullMode
function m.getMeshCullMode()
end

---Gets the height in pixels of the window.
---
---The graphics coordinate system and DPI scale factor, rather than raw pixels. Use getHeight for calculations related to drawing to the screen and using the coordinate system (calculating the center of the screen, for example), and getPixelHeight only when dealing specifically with underlying pixels (pixel-related calculations in a pixel LoveShader, for example).
---@return number
function m.getPixelHeight()
end

---Gets the width in pixels of the window.
---
---The graphics coordinate system and DPI scale factor, rather than raw pixels. Use getWidth for calculations related to drawing to the screen and using the coordinate system (calculating the center of the screen, for example), and getPixelWidth only when dealing specifically with underlying pixels (pixel-related calculations in a pixel LoveShader, for example).
---@return number
function m.getPixelWidth()
end

---Gets the point size.
---@return number
function m.getPointSize()
end

---Gets information about the system's video card and drivers.
---@return string, string, string, string
function m.getRendererInfo()
end

---Gets the current scissor box.
---@return number, number, number, number
function m.getScissor()
end

---Gets the current LoveShader. Returns nil if none is set.
---@return LoveShader
...
---
---Each LoveCanvas has its own per-pixel stencil values.
---@return CompareMode, number
function m.getStencilTest()
end
...
---@return table
function m.getSupported()
end

---Gets the system-dependent maximum values for love.graphics features.
---@return table
function m.getSystemLimits()
end

---Gets the available texture types, and whether each is supported.
---@return table
function m.getTextureTypes()
end
...
function m.intersectScissor(x, y, width, height)
end

---Converts the given 2D position from screen-space into global coordinates.
---
---This effectively applies the reverse of the current graphics transformations to the given position. A similar LoveTransform:inverseTransformPoint method exists for LoveTransform objects.
...
---@return number, number
function m.inverseTransformPoint(screenX, screenY)
end
...
---@return boolean
function m.isGammaCorrect()
end

---Gets whether wireframe mode is used when drawing.
---@return boolean
function m.isWireframe()
end
...
---
---A specific layer of an array image can be drawn with love.graphics.drawLayer / LoveSpriteBatch:addLayer, or with the LoveQuad variant of love.graphics.draw and LoveQuad:setLayer, or via a custom LoveShader.
---
---To use an array image in a LoveShader, it must be declared as a ArrayImage or sampler2DArray type (instead of LoveImage or sampler2D). The Texel(ArrayImage image, vec3 texturecoord) shader function must be used to get pixel colors from a slice of the array image. The vec3 argument contains the texture coordinate in the first two components, and the 0-based slice index in the third component.
---@param slices table @A table containing filepaths to images (or LoveFile, LoveFileData, LoveImageData, or LoveCompressedImageData objects), in an array. Each sub-image must have the same dimensions. A table of tables can also be given, where each sub-table contains all mipmap levels for the slice index of that sub-table.
---@param settings table @Optional table of settings to configure the array image, containing the following fields:
---@return LoveImage
function m.newArrayImage(slices, settings)
end

---Creates a new LoveCanvas object for offscreen rendering.
---@return LoveCanvas
---@overload fun(width:number, height:number):LoveCanvas
---@overload fun(width:number, height:number, settings:table):LoveCanvas
---@overload fun(width:number, height:number, layers:number, settings:table):LoveCanvas
function m.newCanvas()
end

---Creates a new cubemap LoveImage.
---
---Cubemap images have 6 faces (sides) which represent a cube. They can't be rendered directly, they can only be used in LoveShader code (and sent to the shader via LoveShader:send).
---
---To use a cubemap image in a LoveShader, it must be declared as a CubeImage or samplerCube type (instead of LoveImage or sampler2D). The Texel(CubeImage image, vec3 direction) shader function must be used to get pixel colors from the cubemap. The vec3 argument is a normalized direction from the center of the cube, rather than explicit texture coordinates.
---
...
---
---+x -x +y -y +z -z
---@param filename string @The filepath to a cubemap image file (or a LoveFile, LoveFileData, or LoveImageData).
---@param settings table @Optional table of settings to configure the cubemap image, containing the following fields:
---@return LoveImage
---@overload fun(faces:table, settings:table):LoveImage
function m.newCubeImage(filename, settings)
end

---Creates a new LoveFont from a TrueType LoveFont or BMFont file. Created fonts are not cached, in that calling this function with the same arguments will always create a new LoveFont object.
---
---All variants which accept a filename can also accept a LoveData object instead.
---@param filename string @The filepath to the BMFont or TrueType font file.
---@return LoveFont
---@overload fun(filename:string, size:number, hinting:HintingMode, dpiscale:number):LoveFont
---@overload fun(filename:string, imagefilename:string):LoveFont
---@overload fun(size:number, hinting:HintingMode, dpiscale:number):LoveFont
function m.newFont(filename)
end

---Creates a new LoveImage from a filepath, LoveFileData, an LoveImageData, or a LoveCompressedImageData, and optionally generates or specifies mipmaps for the image.
---@param filename string @The filepath to the image file.
---@return LoveImage
---@overload fun(imageData:LoveImageData):LoveImage
---@overload fun(compressedImageData:LoveCompressedImageData):LoveImage
---@overload fun(filename:string, flags:table):LoveImage
function m.newImage(filename)
end
...
---@return LoveFont
---@overload fun(imageData:LoveImageData, glyphs:string):LoveFont
---@overload fun(filename:string, glyphs:string, extraspacing:number):LoveFont
function m.newImageFont(filename, glyphs)
end

---Creates a new LoveMesh.
---
---Use LoveMesh:setTexture if the LoveMesh should be textured with an LoveImage or LoveCanvas when it's drawn.
---
---In versions prior to 11.0, color and byte component values were within the range of 0 to 255 instead of 0 to 1.
...
---@param usage SpriteBatchUsage @The expected usage of the LoveMesh. The specified usage mode affects the LoveMesh's memory usage and performance.
---@return LoveMesh
---@overload fun(vertexcount:number, mode:MeshDrawMode, usage:SpriteBatchUsage):LoveMesh
---@overload fun(vertexformat:table, vertices:table, mode:MeshDrawMode, usage:SpriteBatchUsage):LoveMesh
---@overload fun(vertexformat:table, vertexcount:number, mode:MeshDrawMode, usage:SpriteBatchUsage):LoveMesh
---@overload fun(vertexcount:number, texture:LoveTexture, mode:MeshDrawMode):LoveMesh
function m.newMesh(vertices, mode, usage)
end

---Creates a new LoveParticleSystem.
---@param image LoveImage @The image to use.
---@param buffer number @The max number of particles at the same time.
---@return LoveParticleSystem
---@overload fun(texture:LoveTexture, buffer:number):LoveParticleSystem
function m.newParticleSystem(image, buffer)
end

---Creates a new LoveQuad.
---
---The purpose of a LoveQuad is to use a fraction of an image to draw objects, as opposed to drawing entire image. It is most useful for sprite sheets and atlases: in a sprite atlas, multiple sprites reside in same image, quad is used to draw a specific sprite from that image; in animated sprites with all frames residing in the same image, quad is used to draw specific frame from the animation.
---@param x number @The top-left position in the LoveImage along the x-axis.
---@param y number @The top-left position in the LoveImage along the y-axis.
---@param width number @The width of the LoveQuad in the LoveImage. (Must be greater than 0.)
---@param height number @The height of the LoveQuad in the LoveImage. (Must be greater than 0.)
---@param sw number @The reference width, the width of the LoveImage. (Must be greater than 0.)
---@param sh number @The reference height, the height of the LoveImage. (Must be greater than 0.)
---@return LoveQuad
function m.newQuad(x, y, width, height, sw, sh)
end

---Creates a new LoveShader object for hardware-accelerated vertex and pixel effects. A LoveShader contains either vertex shader code, pixel shader code, or both.
---
---Shaders are small programs which are run on the graphics card when drawing. Vertex shaders are run once for each vertex (for example, an image has 4 vertices - one at each corner. A LoveMesh might have many more.) Pixel shaders are run once for each pixel on the screen which the drawn object touches. Pixel shader code is executed after all the object's vertices have been processed by the vertex shader.
---@param code string @The pixel shader or vertex shader code, or a filename pointing to a file with the code.
---@return LoveShader
---@overload fun(pixelcode:string, vertexcode:string):LoveShader
function m.newShader(code)
end

---Creates a new LoveSpriteBatch object.
---@param image LoveImage @The LoveImage to use for the sprites.
---@param maxsprites number @The maximum number of sprites that the LoveSpriteBatch can contain at any given time. Since version 11.0, additional sprites added past this number will automatically grow the spritebatch.
---@return LoveSpriteBatch
---@overload fun(image:LoveImage, maxsprites:number, usage:SpriteBatchUsage):LoveSpriteBatch
---@overload fun(texture:LoveTexture, maxsprites:number, usage:SpriteBatchUsage):LoveSpriteBatch
function m.newSpriteBatch(image, maxsprites)
end

---Creates a new drawable LoveText object.
---@param font LoveFont @The font to use for the text.
---@param textstring string @The initial string of text that the new LoveText object will contain. May be nil.
---@return LoveText
function m.newText(font, textstring)
end

---Creates a new drawable LoveVideo. Currently only Ogg Theora video files are supported.
---@param filename string @The file path to the Ogg Theora video file.
---@return LoveVideo
---@overload fun(videostream:LoveVideoStream):LoveVideo
---@overload fun(filename:string, settings:table):LoveVideo
---@overload fun(filename:string, loadaudio:boolean):LoveVideo
---@overload fun(videostream:LoveVideoStream, loadaudio:boolean):LoveVideo
function m.newVideo(filename)
end

---Creates a new volume (3D) LoveImage.
---
---Volume images are 3D textures with width, height, and depth. They can't be rendered directly, they can only be used in LoveShader code (and sent to the shader via LoveShader:send).
---
---To use a volume image in a LoveShader, it must be declared as a VolumeImage or sampler3D type (instead of LoveImage or sampler2D). The Texel(VolumeImage image, vec3 texcoords) shader function must be used to get pixel colors from the volume image. The vec3 argument is a normalized texture coordinate with the z component representing the depth to sample at (ranging from 1).
---
...
---@param layers table @A table containing filepaths to images (or LoveFile, LoveFileData, LoveImageData, or LoveCompressedImageData objects), in an array. A table of tables can also be given, where each sub-table represents a single mipmap level and contains all layers for that mipmap.
---@param settings table @Optional table of settings to configure the volume image, containing the following fields:
---@return LoveImage
function m.newVolumeImage(layers, settings)
end
...
---Draws text on screen. If no LoveFont is set, one will be created and set (once) if needed.
---
---As of LOVE 0.7.1, when using translation and scaling functions while drawing text, this function assumes the scale occurs first.  If you don't script with this in mind, the text won't be in the right position, or possibly even on screen.
---
---love.graphics.print and love.graphics.printf both support UTF-8 encoding. You'll also need a proper LoveFont for special characters.
---
---In versions prior to 11.0, color and byte component values were within the range of 0 to 255 instead of 0 to 1.
...
---@overload fun(text:string, transform:LoveTransform):void
---@overload fun(coloredtext:table, transform:LoveTransform):void
---@overload fun(text:string, font:LoveFont, transform:LoveTransform):void
---@overload fun(coloredtext:table, font:LoveFont, transform:LoveTransform):void
function m.print(text, x, y, r, sx, sy, ox, oy, kx, ky)
end
...
---@param kx number @Shearing factor (x-axis).
---@param ky number @Shearing factor (y-axis).
---@overload fun(text:string, font:LoveFont, x:number, y:number, limit:number, align:AlignMode, r:number, sx:number, sy:number, ox:number, oy:number, kx:number, ky:number):void
---@overload fun(text:string, transform:LoveTransform, limit:number, align:AlignMode):void
---@overload fun(text:string, font:LoveFont, transform:LoveTransform, limit:number, align:AlignMode):void
...
---@overload fun(coloredtext:table, font:LoveFont, x:number, y:number, limit:number, align:AlignMode, angle:number, sx:number, sy:number, ox:number, oy:number, kx:number, ky:number):void
---@overload fun(coloredtext:table, transform:LoveTransform, limit:number, align:AlignMode):void
---@overload fun(coloredtext:table, font:LoveFont, transform:LoveTransform, limit:number, align:AlignMode):void
function m.printf(text, x, y, limit, align, r, sx, sy, ox, oy, kx, ky)
end
...
---Replaces the current coordinate transformation with the given LoveTransform object.
---@param transform LoveTransform @The LoveTransform object to replace the current graphics coordinate transform with.
function m.replaceTransform(transform)
end
...
function m.setBackgroundColor(red, green, blue, alpha)
end
...
---Captures drawing operations to a LoveCanvas.
---@param canvas LoveCanvas @The new target.
---@param mipmap number @The mipmap level to render to, for Canvases with mipmaps.
---@overload fun(canvas1:LoveCanvas, canvas2:LoveCanvas, ...:LoveCanvas):void
---@overload fun(canvas:LoveCanvas, slice:number, mipmap:number):void
---@overload fun(setup:table):void
function m.setCanvas(canvas, mipmap)
end
...
function m.setColorMask(red, green, blue, alpha)
end
...
---@param anisotropy number @Maximum amount of Anisotropic Filtering used.
function m.setDefaultFilter(min, mag, anisotropy)
end
...
function m.setDepthMode(comparemode, write)
end

---Set an already-loaded LoveFont as the current font or create and load a new one from the file and size.
---
---It's recommended that LoveFont objects are created with love.graphics.newFont in the loading stage and then passed to this function in the drawing stage.
---@param font LoveFont @The LoveFont object to use.
...
function m.setFrontFaceWinding(winding)
end
...
---Sets whether back-facing triangles in a LoveMesh are culled.
---
...
---@param mode CullMode @The LoveMesh face culling mode to use (whether to render everything, cull back-facing triangles, or cull front-facing triangles).
function m.setMeshCullMode(mode)
end

---Creates and sets a new LoveFont.
---@param size number @The size of the font.
---@return LoveFont
---@overload fun(filename:string, size:number):LoveFont
---@overload fun(file:LoveFile, size:number):LoveFont
---@overload fun(data:LoveData, size:number):LoveFont
---@overload fun(rasterizer:Rasterizer):LoveFont
function m.setNewFont(size)
end
...
function m.setScissor(x, y, width, height)
end

---Sets or resets a LoveShader as the current pixel effect or vertex shaders. All drawing operations until the next ''love.graphics.setShader'' will be drawn using the LoveShader object specified.
---@param shader LoveShader @The new shader.
...
function m.stencil(stencilfunction, action, value, keepvalues)
end

---Converts the given 2D position from global coordinates into screen-space.
---
---This effectively applies the current graphics transformations to the given position. A similar LoveTransform:transformPoint method exists for LoveTransform objects.
...
---@return number, number
function m.transformPoint(globalX, globalY)
end
...
function m.validateShader(gles, code)
end
