...
-- region LoveCompressedImageData
---@class LoveCompressedImageData
---Represents compressed image data designed to stay compressed in RAM.
---
---LoveCompressedImageData encompasses standard compressed texture formats such as  DXT1, DXT5, and BC5 / 3Dc.
---
---You can't draw LoveCompressedImageData directly to the screen. See LoveImage for that.
local LoveCompressedImageData = {}
---Gets the width and height of the LoveCompressedImageData.
---@return number, number
---@overload fun(level:number):number, number
function LoveCompressedImageData:getDimensions()
end

---Gets the format of the LoveCompressedImageData.
---@return CompressedImageFormat
function LoveCompressedImageData:getFormat()
end

---Gets the height of the LoveCompressedImageData.
---@return number
---@overload fun(level:number):number
function LoveCompressedImageData:getHeight()
end

---Gets the number of mipmap levels in the LoveCompressedImageData. The base mipmap level (original image) is included in the count.
---@return number
function LoveCompressedImageData:getMipmapCount()
end

---Gets the width of the LoveCompressedImageData.
---@return number
---@overload fun(level:number):number
function LoveCompressedImageData:getWidth()
end

-- endregion LoveCompressedImageData
-- region LoveImageData
---@class LoveImageData
---Raw (decoded) image data.
---
---You can't draw LoveImageData directly to screen. See LoveImage for that.
local LoveImageData = {}
---Encodes the LoveImageData and optionally writes it to the save directory.
---@param format ImageFormat @The format to encode the image as.
---@param filename string @The filename to write the file to. If nil, no file will be written but the LoveFileData will still be returned.
---@return LoveFileData
---@overload fun(outFile:string):void
---@overload fun(outFile:string, format:ImageFormat):void
function LoveImageData:encode(format, filename)
end

---Gets the width and height of the LoveImageData in pixels.
---@return number, number
function LoveImageData:getDimensions()
end

---Gets the height of the LoveImageData in pixels.
---@return number
...
---Gets the width of the LoveImageData in pixels.
---@return number
function LoveImageData:getWidth()
end

---LoveTransform an image by applying a function to every pixel.
---
---This function is a higher-order function. It takes another function as a parameter, and calls it once for each pixel in the LoveImageData.
---
...
---@param x number @The x-axis of the top-left corner of the area within the LoveImageData to apply the function to.
---@param y number @The y-axis of the top-left corner of the area within the LoveImageData to apply the function to.
---@param width number @The width of the area within the LoveImageData to apply the function to.
---@param height number @The height of the area within the LoveImageData to apply the function to.
function LoveImageData:mapPixel(pixelFunction, x, y, width, height)
end

---Paste into LoveImageData from another source LoveImageData.
---@param source LoveImageData @LoveSource LoveImageData from which to copy.
---@param dx number @Destination top-left position on x-axis.
---@param dy number @Destination top-left position on y-axis.
---@param sx number @LoveSource top-left position on x-axis.
---@param sy number @LoveSource top-left position on y-axis.
---@param sw number @LoveSource width.
---@param sh number @LoveSource height.
function LoveImageData:paste(source, dx, dy, sx, sy, sw, sh)
end
...
CompressedImageFormat = {
    ---The DXT1 format. RGB data at 4 bits per pixel (compared to 32 bits for LoveImageData and regular Images.) Suitable for fully opaque images on desktop systems.
    ["DXT1"] = 1,
...
---Pixel formats for Textures, LoveImageData, and LoveCompressedImageData.
PixelFormat = {
...
---Determines whether a file can be loaded as LoveCompressedImageData.
---@param filename string @The filename of the potentially compressed image file.
---@return boolean
---@overload fun(fileData:LoveFileData):boolean
function m.isCompressed(filename)
end

---Create a new LoveCompressedImageData object from a compressed image file. LÖVE supports several compressed texture formats, enumerated in the CompressedImageFormat page.
---@param filename string @The filename of the compressed image file.
---@return LoveCompressedImageData
---@overload fun(fileData:LoveFileData):LoveCompressedImageData
function m.newCompressedData(filename)
end

---Creates a new LoveImageData object.
---@param width number @The width of the LoveImageData.
---@param height number @The height of the LoveImageData.
---@return LoveImageData
---@overload fun(width:number, height:number, format:PixelFormat, data:string):LoveImageData
---@overload fun(width:number, height:number, data:string):LoveImageData
---@overload fun(filename:string):LoveImageData
---@overload fun(filedata:LoveFileData):LoveImageData
function m.newImageData(width, height)
end
